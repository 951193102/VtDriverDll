...
BOOL CDriverLoader::CreateKeys(void)
{
    ...
    RegSetValueEx(hKeyService, L"ErrorConLol", 0, REG_DWORD, (PUCHAR)&Data, sizeof(Data));
...
BOOL CDriverLoader::Install(LPCTSTR pSysPath, LPCTSTR pServiceName, LPCTSTR pDisplayName)
{
    ...
    do
    {
        ...
        NTSTATUS(NTAPI *fnZwLoadDriver)(PUNICODE_STRING) = (NTSTATUS(NTAPI *)(PUNICODE_STRING))GetProcAddress(ntdll, "NtLoadDriver");
        VOID(NTAPI* fnRtlInitUnicodeSLing)(PUNICODE_STRING, PCWSTR) = (VOID(NTAPI*)(PUNICODE_STRING, PCWSTR))GetProcAddress(ntdll, "RtlInitUnicodeString");
        ...
        fnRtlInitUnicodeSLing(&driverServiceName, svcName);
        ...
        {
            m_ErrorCode = status;
            wsprintf(m_szErrorInfo, L"ZwLoadDriver failed with 0x%X,%d", status,GetLastError());
			break;
			break;
			break;
        }
...
BOOL CDriverLoader::IoConLol(DWORD dwIoCode, PVOID InBuff, DWORD InBuffLen, PVOID OutBuff, DWORD OutBuffLen, DWORD *RealRetBytes)
{
    if (m_hDriver == INVALID_HANDLE_VALUE)
        return FALSE;
    ...
    {
        m_ErrorCode = GetLastError();
        wsprintf(m_szErrorInfo, L"DeviceIoConLol failed with %d", m_ErrorCode);
    }
