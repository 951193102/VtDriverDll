...
void GetThreadFlagsString(SVC_ThreadFlags flags, std::string &str)
{
...
void CUniqueEvent_WithStatusResult::GetBriefResult(std::string &str) const
{
	str = (boost::format("0x%1% (%2%)") % FormatHexString(m_ResultStatus, 8) % GetNTStatusCodeString(m_ResultStatus)).str();/*std::string("0x%1 (%2)").arg(
...
void CUniqueEvent_WithTargetProcess::GetBriefArgument(std::string &str) const
{
    str = m_TargetProcess->GetDisplayName();
}

void CUniqueEvent_WithTargetProcess::GetFullArgument(std::string &str) const
{
	str = (boost::format("Process:\t%1%") % m_TargetProcess->GetDisplayNameWithPID()).str(); /*QObject::tr("Process:\t%1").arg(m_TargetProcess->GetDisplayNameWithPID());*/
}
...
void CUniqueEvent_ProcessCreate::GetBriefArgument(std::string &str) const
{
	if (m_UniqueProcess->m_pParentProcess) {
		str = (boost::format("Parent Process:%1%") % m_UniqueProcess->m_pParentProcess->GetDisplayName()).str();
	}
        //str = QObject::tr("Parent Process:%1").arg(m_UniqueProcess->m_pParentProcess->GetDisplayName());
}

void CUniqueEvent_ProcessCreate::GetFullArgument(std::string &str) const
{
	str = (boost::format("Parent Process:\t%1%\nCommand Line:\t%2%") % (m_UniqueProcess->m_pParentProcess ? m_UniqueProcess->m_pParentProcess->GetDisplayNameWithPID() : std::string()) %
		m_UniqueProcess->m_CommandLine).str();
    /*str = QObject::tr("Parent Process:\t%1\nCommand Line:\t%2")
            .arg(m_UniqueProcess->m_pParentProcess ? m_UniqueProcess->m_pParentProcess->GetDisplayNameWithPID() : std::string(),
...
void CUniqueEvent_CreateProcess::GetBriefArgument(std::string &str) const
{
	str = (boost::format("Create Process:%1%") % m_TargetProcess->GetDisplayName()).str();
    //str = QObject::tr("Create Process:%1").arg(m_TargetProcess->GetDisplayName());
}

void CUniqueEvent_CreateProcess::GetFullArgument(std::string &str) const
{
	str = (boost::format("Create Process:\t%1%\nCommand Line:\t%2%") % m_TargetProcess->GetDisplayNameWithPID() % m_TargetProcess->m_CommandLine).str();
...
void CUniqueEvent_ThreadCreate::GetBriefArgument(std::string &str) const
{
	str = (boost::format("ThreadID: #%1%") % m_ThreadId).str();
    //str = QObject::tr("ThreadID: #%1").arg(m_ThreadId);
}

void CUniqueEvent_ThreadCreate::GetFullArgument(std::string &str) const
{
    std::string ThreadFlagsString;
    GetThreadFlagsString(m_ThreadFlags, ThreadFlagsString);
	str = (boost::format("Parent Process:\t%1%\nParent Thread:\t#%2%\nStart Address:\t0x%3%\nThread Flags:\t%%4%") % m_CreatorProcess->GetDisplayNameWithPID() %
										m_CreatorThreadId %
										FormatHexString(m_StartAddress, (m_UniqueProcess->m_bIs64Bit) ? 16 : 8) %
										ThreadFlagsString).str();
    /*str = QObject::tr("Parent Process:\t%1\nParent Thread:\t#%2\nStart Address:\t0x%3\nThread Flags:\t%4")
            .arg(m_CreatorProcess->GetDisplayNameWithPID(),
                 std::string::number(m_CreatorThreadId),
                 FormatHexString(m_StartAddress, (m_UniqueProcess->m_bIs64Bit) ? 16 : 8),
                 ThreadFlagsString);*/
}
...
void CUniqueEvent_ThreadDestroy::GetBriefArgument(std::string &str) const
{
	str = (boost::format("ThreadID: #%1%") % m_ThreadId).str();
    //str = QObject::tr("ThreadID: #%1").arg(m_ThreadId);
}
...
void CUniqueEvent_CreateThread::GetBriefArgument(std::string &str) const
{
	str = (boost::format("Create Thread #%1% in Process %2%") % m_NewThreadId % m_TargetProcess->GetDisplayName()).str();
   /* str = QObject::tr("Create Thread #%1 in Process %2").arg(
                std::string::number(m_NewThreadId),
                m_TargetProcess->GetDisplayName()
                );*/
}

void CUniqueEvent_CreateThread::GetFullArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetBriefArgument(str);
	str = (boost::format("\nThreadId:\t%1%") % m_NewThreadId).str();
...
void CUniqueEvent_LoadImage::GetFullArgument(std::string &str) const
{
	str = (boost::format("ImageBase:\t0x%1%\nImageSize:\t0x%2%") % FormatHexString(m_ImageBase, (m_UniqueProcess->m_bIs64Bit) ? 16 : 8) % FormatHexString(m_ImageSize, 0)).str();
...
CUniqueEvent_LoadDriver::CUniqueEvent_LoadDriver(CUniqueProcess *up, ULONG ThreadId, LPCWSTR szServiceName, LPCWSTR szImagePath, ULONG ResultStatus, ULONG64 KeSystemTime, ULONG64 EventId) :
    CUniqueEvent_WithStatusResult(up, ThreadId, ResultStatus, KeSystemTime, EventId)
{
	m_ServiceName = (boost::format("%ws") % szServiceName).str();
	m_ImagePath = (boost::format("%ws") % szImagePath).str();
    //m_ServiceName = std::string::fromWCharArray(szServiceName);
   // m_ImagePath = std::string::fromWCharArray(szImagePath);
}

void CUniqueEvent_LoadDriver::GetBriefArgument(std::string &str) const
{
    str = m_ServiceName;
}

void CUniqueEvent_LoadDriver::GetFullArgument(std::string &str) const
{
	str = (boost::format("ServiceName:\t%1%\nImagePath:\t%2%") % m_ServiceName % m_ImagePath).str();
...
void CUniqueEvent_OpenProcess::GetBriefArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetBriefArgument(str);
...
void CUniqueEvent_OpenProcess::GetFullArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetFullArgument(str);

    std::string desiredAccess;
    GetProcessDesiredAccessString(m_DesireAccess, desiredAccess);
	str += (boost::format("\nDesiredAccess:\t%1%") % desiredAccess).str();
...
void CUniqueEvent_OpenThread::GetBriefArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetBriefArgument(str);

    std::string desiredAccess;
    GetThreadDesiredAccessString(m_DesireAccess, desiredAccess);
	str += (boost::format(", ThreadId: #%1%, %2%") % m_TargetThreadId % desiredAccess).str();
    //str += QObject::tr(", ThreadId: #%1, %2").arg(std::string::number(m_TargetThreadId), desiredAccess);
}

void CUniqueEvent_OpenThread::GetFullArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetFullArgument(str);

    std::string desiredAccess;
    GetThreadDesiredAccessString(m_DesireAccess, desiredAccess);
	str += (boost::format("\nThreadId:\t%1%\nDesiredAccess:\t%2%") % m_TargetThreadId % desiredAccess).str();
    //str += QObject::tr("\nThreadId:\t%1\nDesiredAccess:\t%2").arg(std::string::number(m_TargetThreadId), desiredAccess);
}
...
void CUniqueEvent_AllocateVirtualMemory::GetFullArgument(std::string &str) const
{
    std::string str1, protect;
    GetPageProtectionString(m_Protect, protect);
    CUniqueEvent_WithTargetProcess::GetFullArgument(str1);
	str += (boost::format("\nBaseAddress:\t0x%1%\nBufferSize:\t0x%2%\nNewBaseAddress:\t0x%3%\nNewBufferSize:\t0x%4%\nMemType:\t%5%\nProtect:\t%6%") % FormatHexString(m_OldBaseAddress, (m_TargetProcess->m_bIs64Bit || m_OldBaseAddress > 0xffffffff) ? 16 : 8)
		...
		% GetMemoryTypeString(m_AllocationType)
		% protect).str();
...
void CUniqueEvent_ReadVirtualMemory::GetFullArgument(std::string &str) const
{
    std::string str1;
    CUniqueEvent_WithTargetProcess::GetFullArgument(str1);
	str += (boost::format("\nBaseAddress:\t0x%1%\nBufferSize:\t0x%2%") % FormatHexString(m_BaseAddress, (m_TargetProcess->m_bIs64Bit || m_BaseAddress > 0xffffffff) ? 16 : 8) % 
		FormatHexString(m_BufferSize, 0)).str();
   /* str = str1+QObject::tr("\nBaseAddress:\t0x%1\nBufferSize:\t0x%2")
...
void CUniqueEvent_WriteVirtualMemory::GetFullArgument(std::string &str) const
{
    std::string str1;
    CUniqueEvent_WithTargetProcess::GetFullArgument(str1);
	str += (boost::format("\nBaseAddress:\t0x%1%\nBufferSize:\t0x%2%") % FormatHexString(m_BaseAddress, (m_TargetProcess->m_bIs64Bit || m_BaseAddress > 0xffffffff) ? 16 : 8) %
...
void CUniqueEvent_ProtectVirtualMemory::GetFullArgument(std::string &str) const
{
    std::string str1, oldProtect, newProtect;
    GetPageProtectionString(m_OldProtect, oldProtect);
    GetPageProtectionString(m_NewProtect, newProtect);
    CUniqueEvent_WithTargetProcess::GetFullArgument(str1);
	str += (boost::format("\nBaseAddress:\t0x%1%\nBufferSize:\t0x%2%\nNewBaseAddress:\t0x%3%\nNewBufferSize:\t0x%4%\nOldProtect:\t%5%\nNewProtect:\t%6%")
		% FormatHexString(m_OldBaseAddress, (m_TargetProcess->m_bIs64Bit || m_OldBaseAddress > 0xffffffff) ? 16 : 8)
...
void CUniqueEvent_QueryVirtualMemory::GetBriefArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetBriefArgument(str);
...
void CUniqueEvent_QueryVirtualMemory::GetFullArgument(std::string &str) const
{
    CUniqueEvent_WithTargetProcess::GetFullArgument(str);

    int hexWidth = (m_TargetProcess->m_bIs64Bit) ? 16 : 8;
	str += (boost::format("\nBaseAddress:\t0x%1%\nBufferSize:\t0x%2%") % FormatHexString(m_BaseAddress, hexWidth) %
		GetMemoryInformationClassString(m_QueryClass)).str();
   /* str += QObject::tr("\nBaseAddress:\t0x%1\nQueryClass:\t%2")
...
void CUniqueEvent_QueryVirtualMemory_BasicInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryVirtualMemory::GetFullArgument(str);

    std::string allocProtect;
...
CUniqueEvent_QueryVirtualMemory_MappedFileName::CUniqueEvent_QueryVirtualMemory_MappedFileName(
    ...
    CUniqueEvent_QueryVirtualMemory(up, ThreadId, TargetProcess,
                                    ...
                                    KeSystemTime, EventId)
{
	m_MappedFilePath  = (boost::format("%ws") % szMappedFilePath).str();
   // m_MappedFilePath = std::string::fromWCharArray(szMappedFilePath);
}

void CUniqueEvent_QueryVirtualMemory_MappedFileName::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryVirtualMemory::GetFullArgument(str);
	str  += (boost::format("\n\nImageFileName:\t%1%") % m_MappedFilePath).str();
    /*str += QObject::tr("\n\nImageFileName:\t%1")
...
void CUniqueEvent_OpenMutex::GetBriefArgument(std::string &str) const
{
    std::string desiredAccess;
...
void CUniqueEvent_OpenMutex::GetFullArgument(std::string &str) const
{
    std::string desiredAccess;
    GetMutantDesiredAccessString(m_DesiredAccess, desiredAccess);
	str = (boost::format("DesiredAccess:\t%1%") % desiredAccess).str();
    //str = QObject::tr("DesiredAccess:\t%1").arg(DesiredAccess:\t%1);
}
...
void CUniqueEvent_CreateMutex::GetFullArgument(std::string &str) const
{
    CUniqueEvent_OpenMutex::GetFullArgument(str);
	str += (boost::format("\nInitialOwner:\t%1%") % (m_InitialOwner ? ("true") : ("false"))).str();
...
void CUniqueEvent_OpenDirectoryObject::GetBriefArgument(std::string &str) const
{
    std::string desiredAccess;
...
void CUniqueEvent_OpenDirectoryObject::GetFullArgument(std::string &str) const
{
    std::string desiredAccess;
    GetDirectoryObjectDesiredAccessString(m_DesiredAccess, desiredAccess);
	str = (boost::format("DesiredAccess:\t%1%") % desiredAccess).str();
...
void CUniqueEvent_SetWindowsHook::GetBriefResult(std::string &str) const
{
	str = (boost::format("HHook: %1%") % ((m_ResultHHook) ? FormatHexString(m_ResultHHook, 8) : "NULL")).str();
    //str = QObject::tr("HHook: %1").arg((m_ResultHHook) ?FormatHexString(m_ResultHHook, 8) : "NULL");
}

void CUniqueEvent_SetWindowsHook::GetFullArgument(std::string &str) const
{
	
    std::string hookProc;
    CUniqueModule *um = m_UniqueProcess->GetModuleFromAddress(m_HookProc);
    if(um) {
		hookProc = (boost::format("%ws+0x%2% (0x%3%)") % um->m_UniqueImage->m_FileName
			% FormatHexString(m_HookProc - um->m_ImageBase, 0)
			% FormatHexString(m_HookProc, m_UniqueProcess->m_bIs64Bit ? 16 : 8)).str();

  /*      hookProc = std::string("%1+0x%2 (0x%3)")
            .arg(um->m_UniqueImage->m_FileName,
            FormatHexString(m_HookProc - um->m_ImageBase, 0),
            FormatHexString(m_HookProc, m_UniqueProcess->m_bIs64Bit ? 16 : 8));*/
    } else {
		hookProc = (boost::format("0x%1%)") % FormatHexString(m_HookProc, m_UniqueProcess->m_bIs64Bit ? 16 : 8)).str();
       /* hookProc = std::string("0x%1").arg(
                    FormatHexString(m_HookProc, m_UniqueProcess->m_bIs64Bit ? 16 : 8));*/
    }
    ...
    if(m_Module) {
        ...
        {
			moduleInfo = (boost::format("%1% (0x%2%)") 
				% um->m_UniqueImage->m_FileName
				%  FormatHexString(m_Module, m_UniqueProcess->m_bIs64Bit ? 16 : 8)).str();
            //moduleInfo = std::string("%1 (0x%2)")
            //        .arg(um->m_UniqueImage->m_FileName,
            //        FormatHexString(m_Module, m_UniqueProcess->m_bIs64Bit ? 16 : 8));
        } else {
			moduleInfo = (boost::format("0x%1%")
				% FormatHexString(m_Module, m_UniqueProcess->m_bIs64Bit ? 16 : 8)).str();
            //moduleInfo = std::string("0x%1").arg(FormatHexString(m_Module, m_UniqueProcess->m_bIs64Bit ? 16 : 8));
        }
    } else{
       moduleInfo = "NULL";
    }

	str = (boost::format("Hook Type:\t%1%\nHook ThreadId:\t%2%\nHook Procedure:\t%3%\nFlags:\t%4%\nModule:\t%5%\nModule File:\t%6%")
		% GetWindowsHookTypeString(m_HookType)
		% (m_HookThreadId ? boost::lexical_cast<std::string>(m_HookThreadId) : "0 (Global)")
		% hookProc
		% m_Flags
		% moduleInfo
		% m_ModuleFile.GetQString()).str();
   /* str = QObject::tr("Hook Type:\t%1\nHook ThreadId:\t%2\nHook Procedure:\t%3\nFlags:\t%4\nModule:\t%5\nModule File:\t%6")
            .arg(GetWindowsHookTypeString(m_HookType),
            m_HookThreadId ? std::string("%1").arg(m_HookThreadId) : QObject::tr("0 (Global)"),
            hookProc,
            std::string("%1").arg(m_Flags),
            moduleInfo,
...
void CUniqueEvent_FindWindow::GetBriefResult(std::string &str) const
{
	str = (boost::format("Hwnd: %1%") % ((m_ResultHwnd) ? FormatHexString(m_ResultHwnd, 8) : "NULL")).str();
    //str = QObject::tr("Hwnd: %1").arg((m_ResultHwnd) ? FormatHexString(m_ResultHwnd, 8) : "NULL");
}

void CUniqueEvent_FindWindow::GetBriefArgument(std::string &str) const
{
	str = (boost::format("Class:%1%, Window:%2%")
		% m_ClassName.GetQString()
		% m_WindowName.GetQString()).str();
    //str = QObject::tr("Class:%1, Window:%2")
    //        .arg(m_ClassName.GetQString(), m_WindowName.GetQString() );
}

void CUniqueEvent_FindWindow::GetFullArgument(std::string &str) const
{
	str = (boost::format("Hwnd Parent:\t%1%\nHwnd Child:\t%2%\nWindow Class:\t%3%\nWindow Name:\t%4%")
...
void CUniqueEvent_GetWindowText::GetBriefResult(std::string &str) const
{
    str = m_WindowName.GetQString();
}

void CUniqueEvent_GetWindowText::GetBriefArgument(std::string &str) const
{
	str = (boost::format("Hwnd: %1%") % ((m_Hwnd) ? FormatHexString(m_Hwnd, 8) : "NULL")).str();
    //str = QObject::tr("Hwnd: %1").arg((m_Hwnd) ? FormatHexString(m_Hwnd, 8) : "NULL");
}

void CUniqueEvent_GetWindowText::GetFullArgument(std::string &str) const
{
	str = (boost::format("Hwnd:\t%1%\nMaxCount:\t%2%\n\nWindow Name:\t%3%\nResult Count:\t%4%")
 ...
 /*   str = QObject::tr("Hwnd:\t%1\nMaxCount:\t%2\n\nWindow Name:\t%3\nResult Count:\t%4")
            .arg(
                (m_Hwnd) ? FormatHexString(m_Hwnd,8) : "NULL",
                std::string::number(m_MaxCount),
                m_WindowName.GetQString(),
                std::string::number(m_ResultCount)
            );*/
}
...
void CUniqueEvent_GetWindowClass::GetBriefResult(std::string &str) const
{
    str = m_WindowClass.GetQString();
}

void CUniqueEvent_GetWindowClass::GetBriefArgument(std::string &str) const
{
	str = (boost::format("Hwnd: %1%")
		% ((m_Hwnd) ? FormatHexString(m_Hwnd, 8) : "NULL")).str();
    //str = QObject::tr("Hwnd: %1").arg((m_Hwnd) ? FormatHexString(m_Hwnd, 8) : "NULL");
}

void CUniqueEvent_GetWindowClass::GetFullArgument(std::string &str) const
{
	str = (boost::format("Hwnd:\t%1%\nMaxCount:\t%2%\n\nWindow Class:\t%3%\nResult Count:\t%4%")
		% ((m_Hwnd) ? FormatHexString(m_Hwnd, 8) : "NULL")
		% m_MaxCount
...
void CUniqueEvent_CreateFile::GetBriefArgument(std::string &str) const
{
    str = GetCreateDispositionString(m_Disposition);
}

void CUniqueEvent_CreateFile::GetFullArgument(std::string &str) const
{
    std::string shareAccess;
	...
	str = (boost::format("CreateDisposition:\t%1%\nDesiredAccess:\t%2%\nShareAccess:\t%3%\nOptions:\t%4%")
		% GetCreateDispositionString(m_Disposition)
		% desiredAccess
		% shareAccess
		% (options)
		).str();
...
void CUniqueEvent_ReadFile::GetFullArgument(std::string &str) const
{
	str = (boost::format("Length:\t0x%1%\nByteOffset:\t0x%2%")
		% FormatHexString(m_Length, 0)
...
void CUniqueEvent_WriteFile::GetFullArgument(std::string &str) const
{
	str = (boost::format("Length:\t0x%1%\nByteOffset:\t0x%2%")
		% FormatHexString(m_Length, 0)
		% FormatHexString(m_ByteOffset, 0)
...
void CUniqueEvent_CreateFileMapping::GetBriefArgument(std::string &str) const
{
    std::string syncType;
    switch (m_SyncType)
    {
    ...
    //str = std::string("%1, %2").arg(syncType, protection);
}

void CUniqueEvent_CreateFileMapping::GetFullArgument(std::string &str) const
{
    std::string syncType;
    switch (m_SyncType)
    {
...
void CUniqueEvent_QueryFileInformation::GetBriefArgument(std::string &str) const
{
    str = GetFileInformationClass(m_QueryClass);
}

void CUniqueEvent_QueryFileInformation::GetFullArgument(std::string &str) const
{
	str = (boost::format("Query Class:\t%1%")
		% GetFileInformationClass(m_QueryClass)
		).str();
   // str = QObject::tr("Query Class:\t%1").arg(GetFileInformationClass(m_QueryClass));
    ...
    if(m_QueryClass == FileBasicInformation){
       ...
       // QDateTime date;
        std::string crt, lat, lwt, cht, fa;
        //date.setTime_t(FileTimeToUnixTime((FILETIME *)&m_AllInfo.BasicInformation.CreationTime));
		ptime ptt = from_time_t(FileTimeToUnixTime((FILETIME *)&m_AllInfo.BasicInformation.CreationTime));
		crt = to_simple_string(ptt);
        //crt = date.toString("yyyy-MM-dd HH:mm:ss");
		ptt = from_time_t(FileTimeToUnixTime((FILETIME *)&m_AllInfo.BasicInformation.LastAccessTime));
		lat = to_simple_string(ptt);

		ptt = from_time_t(FileTimeToUnixTime((FILETIME *)&m_AllInfo.BasicInformation.LastWriteTime));
		lwt = to_simple_string(ptt);

		ptt = from_time_t(FileTimeToUnixTime((FILETIME *)&m_AllInfo.BasicInformation.ChangeTime));
		cht = to_simple_string(ptt);
    ...
    if(m_QueryClass == FileStandardInformation){
		str = (boost::format("\nAllocationSize:\t%1%\nEndOfFile:\t%2%\nDeletePending:\t%3%\nDirectory:\t%4%")
			...
			% (m_AllInfo.StandardInformation.DeletePending ? ("true") :("false"))
			% (m_AllInfo.StandardInformation.Directory ? ("true") :("false"))
			).str();
        //str += QObject::tr("\nAllocationSize:\t%1\nEndOfFile:\t%2\nDeletePending:\t%3\nDirectory:\t%4").arg(
        //            FormatFileSizeString(m_AllInfo.StandardInformation.AllocationSize.QuadPart),
        //            FormatFileSizeString(m_AllInfo.StandardInformation.EndOfFile.QuadPart),
...
void CUniqueEvent_OpenKey::GetBriefArgument(std::string &str) const
{
    std::string desiredAccess;
...
void CUniqueEvent_OpenKey::GetFullArgument(std::string &str) const
{
    std::string desiredAccess;
    GetKeyDesiredAccessString(m_DesiredAccess, desiredAccess);
	str = (boost::format("DesiredAccess:\t%1%")
		% desiredAccess
		).str();
    //str = QObject::tr("DesiredAccess:\t%1").arg(desiredAccess);
}
...
void CUniqueEvent_CreateKey::GetFullArgument(std::string &str) const
{
	...
	str = (boost::format("\nDisposition:\t%1%\nCreateOptions:\t%2%")
		% GetCreateKeyDispositionString(m_Disposition)
		).str();
    //str = QObject::tr("\nDisposition:\t%1\nCreateOptions:\t%2")
...
CUniqueEvent_SetValueKey::CUniqueEvent_SetValueKey(CUniqueProcess *up, ULONG ThreadId,
                        LPCWSTR szKeyPath, LPCWSTR szValueName, ULONG DataType, ULONG DataSize, std::string &BinaryData,
                        ULONG ResultStatus, ULONG64 KeSystemTime, ULONG64 EventId) :
    CUniqueEvent_WithTargetFile(up, ThreadId, szKeyPath, ResultStatus, KeSystemTime, EventId)
{
    m_KeyValue = NULL;
...
void CUniqueEvent_SetValueKey::GetBriefArgument(std::string &str) const
{
    str = GetRegistryKeyDataType(m_KeyValue->GetType());
}

void CUniqueEvent_SetValueKey::GetFullArgument(std::string &str) const
{
	str = (boost::format("ValueName:\t%1%\n\nDataType:\t%2%\nDataSize:\t%3%\nData:")
...
CUniqueEvent_QueryValueKey::CUniqueEvent_QueryValueKey(CUniqueProcess *up, ULONG ThreadId,
                                                       LPCWSTR szKeyPath, LPCWSTR szValueName, ULONG QueryClass, ULONG QueryLength, std::string &BinaryData,
                                                       ULONG ResultStatus, ULONG64 KeSystemTime, ULONG64 EventId) :
    CUniqueEvent_WithTargetFile(up, ThreadId, szKeyPath, ResultStatus, KeSystemTime, EventId)
{
...
void CUniqueEvent_QueryValueKey::GetBriefArgument(std::string &str) const
{
    str = GetRegistryQueryValueKeyClass(m_QueryClass);
}

void CUniqueEvent_QueryValueKey::GetFullArgument(std::string &str) const
{
...
inline ULONG GetByteArrayMaxLength(std::string &ba, ULONG myOffset, ULONG myLength)
{
...
CUniqueEvent_QueryValueKey_BasicInformation::CUniqueEvent_QueryValueKey_BasicInformation(
        ...
        LPCWSTR szKeyPath, LPCWSTR szValueName, ULONG QueryLength, std::string &BinaryData,
        ULONG ResultStatus, ULONG64 KeSystemTime, ULONG64 EventId) :
...
void CUniqueEvent_QueryValueKey_BasicInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryValueKey::GetFullArgument(str);

    if(m_KeyType != (ULONG)-1){
		str += (boost::format("Type:\t%1%\nName:\t%2%")
                               ...
                               ResultStatus, KeSystemTime, EventId)
{
    ...
    if(len > 0)
		...
		tempData = std::string((PCHAR)p + p->DataOffset, len);
...
void CUniqueEvent_QueryValueKey_FullInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryValueKey::GetFullArgument(str);
    if(m_KeyValue){
		str += (boost::format("Type:\t%1%\nName:\t%2%\nDataLength:\t0x%3%\nData:")
                               ...
                               ResultStatus, KeSystemTime, EventId)
{
    ...
    if(len > 0)
        tempData = std::string((PCHAR)p->Data, len);
...
void CUniqueEvent_QueryValueKey_PartialInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryValueKey::GetFullArgument(str);
    if(m_KeyValue){
		str += (boost::format("Type:\t%1\nDataLength:\t0x%2\nData:")
...
CUniqueEvent_QueryKey::CUniqueEvent_QueryKey(CUniqueProcess *up, ULONG ThreadId,
                        LPCWSTR szKeyPath, ULONG QueryClass, ULONG QueryLength, std::string &BinaryData,
                        ULONG ResultStatus, ULONG64 KeSystemTime, ULONG64 EventId) :
    CUniqueEvent_WithTargetFile(up, ThreadId, szKeyPath, ResultStatus, KeSystemTime, EventId)
{
    UNREFERENCED_PARAMETER(BinaryData);
...
void CUniqueEvent_QueryKey::GetBriefArgument(std::string &str) const
{
    str = GetRegistryQueryKeyClass(m_QueryClass);
}

void CUniqueEvent_QueryKey::GetFullArgument(std::string &str) const
{
	str += (boost::format("KeyInformationClass:\t%1%\nLength:\t0x%2%\n\n")
...
CUniqueEvent_QueryKey_BasicInformation::CUniqueEvent_QueryKey_BasicInformation(
    CUniqueProcess *up, ULONG ThreadId,
    LPCWSTR szKeyPath, ULONG QueryLength, std::string &BinaryData,
    ULONG ResultStatus, ULONG64 KeSystemTime, ULONG64 EventId) :
...
void CUniqueEvent_QueryKey_BasicInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryKey::GetFullArgument(str);

   /* QDateTime date;
    date.setTime_t(FileTimeToUnixTime((FILETIME *)&m_LastWriteTime));
    std::string dateStr = date.toString("yyyy-MM-dd HH:mm:ss");*/
	using namespace boost::posix_time;
	using namespace boost::gregorian;
	ptime ppt = from_time_t(FileTimeToUnixTime((FILETIME *)&m_LastWriteTime));
	std::string dateStr = to_simple_string(ppt);
	str += (boost::format("LastWriteTime:\t%1%\nKeyName:\t%2%")
		% dateStr
		% m_KeyName.GetQString()
		).str();
...
void CUniqueEvent_QueryKey_NodeInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryKey::GetFullArgument(str);
...
void CUniqueEvent_QueryKey_FullInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryKey::GetFullArgument(str);
    ...
    //            std::string::number(m_SubKeys),
    //            std::string::number(m_MaxNameLen),
    //            std::string::number(m_MaxClassLen),
    //            std::string::number(m_Values),
    //            std::string::number(m_MaxValueNameLen),
    //            std::string::number(m_MaxValueDataLen));
}
...
void CUniqueEvent_QueryKey_NameInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryKey::GetFullArgument(str);
...
void CUniqueEvent_QueryKey_CachedInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryKey::GetFullArgument(str);
 ...
 /*   str += QObject::tr("LastWriteTime:\t%1\nSubKeys:\t%2\nMaxNameLen:\t%3\nValues:\t%4\nMaxValueNameLen:\t%5\nMaxValueDataLen:\t%6\nNameLength:\t%7").arg(
                ...
                std::string::number(m_MaxValueNameLen),
                std::string::number(m_MaxValueDataLen),
                std::string::number(m_NameLength));*/
}
...
void CUniqueEvent_QueryKey_VirtualizationInformation::GetFullArgument(std::string &str) const
{
    CUniqueEvent_QueryKey::GetFullArgument(str);

	str += (boost::format("VirtualizationCandidate:\t%1%\nVirtualizationEnabled:\t%2%\nVirtualTarget:\t%3%\nVirtualStore:\t%4%\nVirtualSource:\t%5%")
