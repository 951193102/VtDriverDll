...
CUniqueImage::CUniqueImage(ULONG ImageSize, LPCWSTR szImagePath, bool bIs64Bit, bool bIsSystemModule)
{
    ...
    m_FileName = (boost::format("%ws") % (ExtractFileName(szImagePath))).str();
    m_bIs64Bit = bIs64Bit;
...
void CModuleWorker::OnQueuedGetModuleInfo(CUniqueProcess *up, ULONG64 BaseAddress)
{
    ...
    {
		QueuedAddModule(up, ImageBase, ImageSize, ImagePath, Is64Bit ? true : false, false);
    }
}
...
void CModuleWorker::OnQueuedGetImageFileInfo(std::wstring ImagePath)
{
	...
	if (0 != fileSize.QuadPart)
	{
		...
		if (GetFileVersionInfo(ImagePath.c_str(), 0, dwSize, pBlock))
		{
			...
			std::string strSubBlock;
			auto fmt = boost::format("%1%%2%%3%");
			fmt % FormatHexString(pTranslate[1], 2)%
			FormatHexString(pTranslate[0], 2)%
			FormatHexString(pTranslate[3], 2)%
			FormatHexString(pTranslate[2], 2);
			std::string strTemp(fmt.str());

			fmt = boost::format("\\StringFileInfo\\%1%\\FileDescription");
			fmt  % strTemp;
			strSubBlock = fmt.str();//QString("\\StringFileInfo\\%1\\FileDescription").arg(strTemp);
			if (VerQueryValueA(pBlock, (LPCSTR)strSubBlock.c_str(), (LPVOID*)&pVerValue, &nSize))
				fileInfo->fileDesc.assign(pVerValue,nSize);// QString::fromUtf16((const ushort *)pVerValue, nSize);

			fmt = boost::format("\\StringFileInfo\\%1%\\ProductName");
			fmt  % strTemp;
			//strSubBlock = QString("\\StringFileInfo\\%1\\ProductName").arg(strTemp);
			if (VerQueryValue(pBlock, (LPCWSTR)strSubBlock.c_str(), (LPVOID*)&pVerValue, &nSize))
				fileInfo->productName.assign(pVerValue, nSize); //fileInfo->productName = QString::fromUtf16((const ushort *)pVerValue, nSize);
			
			fmt = boost::format("\\StringFileInfo\\%1%\\LegalCopyright");
			fmt  % strTemp;
			//strSubBlock = QString("\\StringFileInfo\\%1\\LegalCopyright").arg(strTemp);
			if (VerQueryValue(pBlock, (LPCWSTR)strSubBlock.c_str(), (LPVOID*)&pVerValue, &nSize))
				fileInfo->copyRights.assign(pVerValue, nSize); //fileInfo->copyRights = QString::fromUtf16((const ushort *)pVerValue, nSize);
			fmt = boost::format("\\StringFileInfo\\%1%\\CompanyName");
			fmt  % strTemp;
			//strSubBlock = QString("\\StringFileInfo\\%1\\CompanyName").arg(strTemp);
			if (VerQueryValue(pBlock, (LPCWSTR)strSubBlock.c_str(), (LPVOID*)&pVerValue, &nSize))
				fileInfo->companyName.assign(pVerValue, nSize); // fileInfo->companyName = QString::fromUtf16((const ushort *)pVerValue, nSize);
		    }
		}
		delete pBlock;
	    }
...
void CModuleMgr::QueuedLoadImage(std::wstring ImagePath, bool bUseSymbol, bool bIsSystem)
{
	ULONG ImageSize = 0;
	bool bSuccess = false;
	bool bIs64Bit = false;
	...
	if (bSuccess)
		OnQueuedAddImage(ImagePath, ImageSize, bIs64Bit, bIsSystem, funcList);
	else
		//QueuedLoadImageComplete(ImagePath, NULL);
		BOOST_LOG_TRIVIAL(info) << "Load Image:"<<ImagePath.c_str()<<" Finish.";
}

void CModuleMgr::LoadingImageNotify(std::wstring moduleName)
{
	//
}

void CModuleMgr::QueuedGetModuleInfo(CUniqueProcess * up, ULONG64 BaseAddress)
{
	ULONG64 ImageBase;
	ULONG ImageSize;
	BOOLEAN Is64Bit;
	...
	if (GetImageBaseInfoByAddress(up->m_ProcessId, BaseAddress, &ImageBase, &ImageSize, &Is64Bit) &&
		GetImagePathByAddress(up->m_ProcessId, ImageBase, ImagePath))
	{
		OnQueuedAddModule(up, ImageBase, ImageSize, ImagePath, Is64Bit ? true : false, false);
	}
}

void CModuleMgr::QueuedGetImageFileInfo(std::wstring ImagePath)
{
}
